...
Dense-network entry in the `mnist_classifier` competition. Architecture:
`Dense(512) → Dropout → Dense(256) → Dropout → Dense(10, softmax)` on flattened
28×28 MNIST images.
...
Team 1-specific packages there, but never re-pin a base dependency, or your
...
`@team1-candidate` to point at it. Team 2's `@team2-candidate` alias is
completely separate; you can't clobber each other.

Re-running `train.py` again later just moves `@team1-candidate` to whichever
run wins that time — there is never more than one live Team 1 candidate.
...
it once and store it as the `SECRET_TEST_DATA_B64` GitHub Actions secret —
this is what CI evaluates every candidate against:
...
```bash
cd ../shared
python quality_gate.py --mode evaluate
```

This reuses your `.env` credentials (no separate CI identity needed locally),
pulls your `@team1-candidate` from dev-tracking, compares it against the
current `@champion` on prod-tracking, and prints PASS/FAIL — the exact same
check CI runs on your PR. Works from any working directory.
