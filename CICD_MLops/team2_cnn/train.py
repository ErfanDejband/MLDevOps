...
CANDIDATE_ALIAS = "team2-candidate"
...
def setup_mlflow():
    ...
    if not tracking_uri:
        raise EnvironmentError(
            ...
            "  Copy CICD_MLops/WIKI/.env.example → CICD_MLops/team2_cnn/.env and fill in\n"
    ...
    mlflow.set_experiment("team2_cnn")
...
def load_data():
    ...
    # Normalize pixel values to [0, 1] and keep the 28x28 spatial shape,
    # with an explicit channel dim for Conv2D: (N, 28, 28, 1)
    X_full = X_full.astype("float32") / 255.0
    X_images = X_full.reshape(-1, 28, 28, 1)

    # Split 1: 90% temp, 10% secret test
    X_temp, X_test, y_temp, y_test = train_test_split(
        X_images, y_full, test_size=0.10, random_state=42, stratify=y_full
    )
    ...
    # TODO: this is just an example, in a real situation we should have one test data and process it for different models.
...
if __name__ == "__main__":
    ...
    best_model        = None
    summary = []
    build_model().summary(print_fn=summary.append)
    ...
    for i, params in enumerate(all_param_combinations):
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            mlflow.log_param("architecture", "Conv32-Pool-Conv64-Pool-Flatten-Dense128-Drop-Softmax10")
            mlflow.log_metric("val_accuracy", val_accuracy)
            mlflow.log_metric("val_loss", val_loss)
            mlflow.set_tag("team", "team2_cnn")
            mlflow.set_tag("dataset", "mnist")
            mlflow.log_text("\n".join(summary), "model_summary.txt")
